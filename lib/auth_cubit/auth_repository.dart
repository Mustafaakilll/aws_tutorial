...
class AuthRepository {
    ...
    try {
      ...
      if (e.runtimeType.toString().contains('CodeExpiredException')) {
        throw AuthError('Doğrulama Kodunun Süresi Geçmiş.');
          ...
          .toString()
          .contains('CognitoCodeExpiredException')) {
        throw AuthError('Doğrulama Kodunun Süresi Geçmiş.');
  ...
  Future<String> signIn(String username, String password) async {
    try {
      ...
      debugPrint(
          'GIRIS YAPARKEN GELEN HATA ${e.runtimeType} GELEN HATA GIRIS YAPARKEN');
      if (e.runtimeType.toString().contains('UserNotFoundException')) {
        throw AuthError(
            'Kullanici Bulunamadi. Lutfen bilgilerinizi kontrol edin.');
      } else if (e.runtimeType
          .toString()
          .contains('TooManyFailedAttemptsException')) {
        throw AuthError(
            'Cok fazla hatali deneme yaptiniz. Daha sonra deneyin.');
      } else if (e.runtimeType
          .toString()
        ...
        throw AuthError(
            'Kullanici henuz dogrulanmadi. Lutfen gelistirici ile iletisime gecin');
      } else {
        throw AuthError(e.runtimeType.toString());
      }
    }
